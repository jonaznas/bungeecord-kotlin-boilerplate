...
import hazae41.minecraft.kutils.bungee.schedule
...
class Mysql {
    ...
    private val config: Configuration = ConfigurationProvider.getProvider(YamlConfiguration::class.java).load(File(plugin.dataFolder, "config.yml"))

    init {
        plugin.logger.info("TEST: " + config.getString("database.user"))
                ...
                values.forEachIndexed { i, e ->
                    when (e) {
                        ...
                        is Timestamp -> pstmt.setTimestamp(i + 1, e)
                            ...
                            plugin.logger.warning("Invalid mysql-value type converted to string on index $i ($e)")
